function Itgd = freq_response_tensor_qgrid_ttm_iso(omega,w0,w1,k,rho_cp,kzz,kxx,alpha,gamma,Ce,kez,kex,g)
	M = length(k);
    ...
    for mm = 1:M
        ...
        for iw = 1:length(omega)
            ...
            Itgd(iw,mm) = alpha*gamma/kez.*((u(1,1).*v(1,1)./(lambda1-alpha^2)+u(1,2).*v(2,1)./(lambda2-alpha^2))/(alpha+gamma) ...
